...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($error)) {
        $params = array_merge([$username, $email, $gambar, $id_user], $params);
        
        $sql = "UPDATE users SET username = ?, email = ?, gambar = ? $password_query WHERE id_user = ?";
...
if ($stmt->execute()) {
    ...
    $_SESSION['gambar'] = $gambar; // INI PENTING!
   ...
   // TAMBAHKAN REDIRECT OTOMATIS
    // echo '<meta http-equiv="refresh" content="2;url=../super_admin/index.php?updated=' . time() . '">'
