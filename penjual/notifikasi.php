...
if ($_SESSION['role'] !== 'penjual') {
    ...
    switch ($_SESSION['role']) {
        case 'pembeli':
            header("Location: ../pembeli/notifikasi.php");
            break;
        default:
            header("Location: ../login.php");
            break;
...
// Ambil notifikasi khusus untuk penjual
$sql_notif = "SELECT 
                n.id_notifikasi,
                n.id_user,
                n.id_order,
                ...
                u.username as pembeli_nama
            FROM notifikasi n
            LEFT JOIN transaksi t ON n.id_order = t.id_transaksi
            LEFT JOIN produk p ON n.id_produk = p.id_produk
...
<head>
    ...
    <title>Notifikasi Penjual - Wesley Bookstore</title>
...
<body>
            ...
            <div class="notif-sidebar">   
                <div class="filter-buttons">
                    ...
                    <button class="filter-btn" data-filter="unread" onclick="filterNotifications('unread')">
                        ...
                        Belum Dibaca
                        <span class="filter-badge badge-danger"><?php echo $unread_count; ?></span>
                    </button>
                    
                    <button class="filter-btn" data-filter="new_orders" onclick="filterNotifications('new_orders')">
                        <i class="fas fa-shopping-cart"></i>
                        Pesanan Baru
                        <span class="filter-badge badge-warning"><?php echo $new_orders_count; ?></span>
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function filterNotifications(filter) {
            ...
            const filteredNotifs = notifications.filter(notif => {
                switch(filter) {
                    ...
                    case 'new_orders':
                        return notif.type == 'new_order' || notif.judul == 'Pesanan Baru';
                    case 'approval':
                        return notif.approve_status === 'approve' || 
                               notif.judul === 'Pesanan Disetujui' ||
                               (notif.judul && notif.judul.includes('Disetujui'));
                    case 'rejection':
                        return notif.approve_status === 'reject' || 
                               notif.judul === 'Pesanan Ditolak' ||
            ...
            switch(filter) {
                ...
                case 'unread':
                    ...
                    break;
                case 'new_orders':
                    listTitle.textContent = 'Pesanan Baru';
                    break;
                case 'approval':
        ...
        function generateNotificationHTML(notif) {
            ...
            } else if (notif.approve_status === 'approve' || 
                      notif.judul === 'Pesanan Disetujui' || 
                      (notif.judul && notif.judul.includes('Disetujui'))) {
                iconClass = 'icon-approval';
                statusClass = 'status-approved';
                statusText = 'Disetujui';
            } else if (notif.approve_status === 'reject' || 
                      notif.judul === 'Pesanan Ditolak' ||
                      (notif.judul && notif.judul.includes('Ditolak'))) {
            ...
            return `
                <div class="notif-item ${unreadClass}" data-id="${notif.id_notifikasi}">
                    <div class="notif-icon ${iconClass}">
                        ${getIconByType(notif.type, notif.judul)}
                    </div>
                    <div class="notif-content">
                        ...
                        ${notif.nama_produk ? `
                            <div class="notif-product">
                                ...
                                ${notif.pembeli_nama ? `<span style="color: var(--gray); font-size: 0.85rem;">Dari: ${notif.pembeli_nama}</span>` : ''}
                            </div>
                        ` : ''}
        ...
        // Get icon by type
        function getIconByType(type, title) {
            if (type == 'new_order' || title == 'Pesanan Baru') {
                return '<i class="fas fa-shopping-cart"></i>';
            } else if (title === 'Pesanan Disetujui' || (title && title.includes('Disetujui'))) {
                return '<i class="fas fa-check-circle"></i>';
            } else if (title === 'Pesanan Ditolak' || (title && title.includes('Ditolak'))) {
                return '<i class="fas fa-times-circle"></i>';
            } else if (title && title.includes('Dikirim')) {
        ...
        async function viewNotification(notifId) {
            ...
            if (notif) {
                if (notif.id_order && (notif.type == 'new_order' || notif.judul == 'Pesanan Baru')) {
                    // Redirect to order approval page
                    window.location.href = `approve.php?order_id=${notif.id_order}`;
                } else if (notif.id_order) {
                    // Redirect to transaction detail
                    window.location.href = `transaksi_detail.php?id=${notif.id_order}`;
                }
                // Jika tidak ada link, tetap di halaman notifikasi
            }
        }
        ...
        async function markAllAsRead() {
            try {
                ...
                if (data.success) {
                    ...
                    // Refresh notification system in dashboard
                    if (window.parent && window.parent.penjualNotificationSystem) {
                        window.parent.penjualNotificationSystem.loadInitialCounts();
                    }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Setup polling untuk update realtime setiap 30 detik
            setInterval(() => {
                // Reload page jika ada perubahan
                location.reload();
            }, 30000);
        });
    </script>
