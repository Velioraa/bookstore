...
<body>
    ...
    <div class="chat-header-back">
        <button class="back-button" onclick="window.location.href='status.php'">
        ...
        <div class="chat-sidebar" id="chatSidebar">
            <div class="chat-sidebar-header">
                <div style="display: flex; align-items: center;">
                    ...
                    <div id="sidebarBadgeCounter" class="sidebar-header-badge" style="display: none;">0</div>
                </div>
            ...
            <div class="chat-contacts" id="chatContacts">
                ...
                <?php else: ?>
                    ...
                    foreach ($contacts as $contact): 
                        $totalUnread += $contact['unread_count'];
                    ?>
                    <div class="chat-contact" data-user-id="<?php echo $contact['contact_id']; ?>" onclick="selectContactFromPHP(<?php echo htmlspecialchars(json_encode($contact), ENT_QUOTES, 'UTF-8'); ?>)">
                        ...
                        <div class="contact-meta">
                            ...
                            <div class="contact-unread <?php echo $contact['unread_count'] > 9 ? 'large' : ''; ?>">
                                <?php echo $contact['unread_count'] > 99 ? '99+' : $contact['unread_count']; ?>
                            </div>
                            <?php endif; ?>
                        </div>
                    </div>
...
<script>
    ...
    let unreadMessages = new Map(); // Untuk melacak pesan yang belum dibaca per kontak
    ...
    const sidebarBadgeCounter = document.getElementById('sidebarBadgeCounter');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Inisialisasi unread messages dari PHP contacts
        initializeUnreadMessages();
        
        // Update sidebar badge counter
        updateSidebarBadgeCounter();
    ...
    function initializeUnreadMessages() {
        // Inisialisasi dari contacts PHP
        contactsFromPHP.forEach(contact => {
            if (contact.unread_count > 0) {
                unreadMessages.set(contact.contact_id, contact.unread_count);
            }
        });
    }
    
    function updateUnreadCount(contactId, count) {
        if (count > 0) {
            unreadMessages.set(contactId, count);
        } else {
            unreadMessages.delete(contactId);
        }
        
        // Update UI untuk kontak tertentu
        updateContactUnreadBadge(contactId, count);
    ...
    function updateContactUnreadBadge(contactId, count) {
        ...
        let unreadBadge = contactElement.querySelector('.contact-unread');
        
        if (count > 0) {
            if (!unreadBadge) {
                unreadBadge = document.createElement('div');
                unreadBadge.className = 'contact-unread';
                if (count > 9) unreadBadge.classList.add('large');
                metaElement.appendChild(unreadBadge);
            }
    ...
    function updateSidebarBadgeCounter() {
        ...
        if (totalUnread > 0) {
            sidebarBadgeCounter.textContent = totalUnread > 99 ? '99+' : totalUnread;
            sidebarBadgeCounter.style.display = 'flex';
            
            // Update title tab dengan jumlah pesan baru
            document.title = `(${totalUnread}) Wesley Bookstore - Chat`;
        } else {
            sidebarBadgeCounter.style.display = 'none';
            document.title = 'Wesley Bookstore - Chat';
        }
        
        // Juga update badge di tombol sidebar (jika ada)
        updateMobileSidebarBadge(totalUnread);
    }
    
    function updateMobileSidebarBadge(count) {
        if (toggleSidebarBtn && count > 0) {
            // Tambah badge kecil di tombol sidebar
            let badge = toggleSidebarBtn.querySelector('.sidebar-badge');
            if (!badge) {
            ...
            const badge = toggleSidebarBtn.querySelector('.sidebar-badge');
            if (badge) {
    ...
    async function updateAllUnreadCounts() {
        try {
            ...
            const response = await fetch(
                `${API_URL}?action=get-unread-counts`,
                {
            ...
            if (data.success && data.unread_counts) {
                // Update unread counts untuk semua kontak
                data.unread_counts.forEach(item => {
                    updateUnreadCount(item.contact_id, item.unread_count);
                });
            }
        } catch (error) {
            // Silent fail untuk polling
            console.log('Polling unread counts error:', error.message);
        }
    }
    ...
    function selectContact(contact) {
        ...
        if (contactElement) {
            ...
            // Reset unread badge untuk kontak ini
            updateUnreadCount(contact.contact_id, 0);
        }
        ...
        try {
            ...
            if (data.success && data.messages && data.messages.length > 0) {
                ...
                if (hasNewMessages) {
                    ...
                    // Jika chat ini sedang tidak aktif, update unread count
                    if (!document.hasFocus() || !isChatActive()) {
                        updateUnreadCount(selectedContact.contact_id, 
                            (unreadMessages.get(selectedContact.contact_id) || 0) + newUnreadCount);
                    }
                }
            }
        } catch (error) {
    ...
    function isChatActive() {
        // Cek apakah chat window sedang aktif (visible dan focused)
    ...
    function addContactToList(contact) {
        ...
        contactDiv.innerHTML = `
            ...
            <div class="contact-meta">
                ...
                ${contact.unread_count > 0 ? `<div class="contact-unread ${contact.unread_count > 9 ? 'large' : ''}">${contact.unread_count > 99 ? '99+' : contact.unread_count}</div>` : ''}
            </div>
        `;
        ...
        // Tambah ke unread messages map jika ada
        if (contact.unread_count > 0) {
            updateUnreadCount(contact.contact_id, contact.unread_count);
        }
    }
    ...
    function showBrowserNotification(title, body, contactId) {
        if (!("Notification" in window)) {
            return;
        }
        
        if (Notification.permission === "granted") {
            createNotification(title, body, contactId);
    ...
    function createNotification(title, body, contactId) {
        const notification = new Notification(title, {
            body: body,
            icon: '../asset/wesley.png'
        });
        
        notification.onclick = function() {
            window.focus();
            
            // Coba pilih kontak jika ada
            const contactElement = document.querySelector(`.chat-contact[data-user-id="${contactId}"]`);
            if (contactElement) {
                contactElement.click();
            }
        ...
        // Auto close notification setelah 5 detik
        setTimeout(() => {
            notification.close();
        }, 5000);
    }
    ...
    document.addEventListener('visibilitychange', function() {
        if (document.visibilityState === 'visible') {
            // User kembali ke tab, update unread counts
            updateAllUnreadCounts();
            
            // Jika ada kontak yang dipilih, mark as read
            if (selectedContact) {
                markAsRead(selectedContact.contact_id);
            }
        }
    });
