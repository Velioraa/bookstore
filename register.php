...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Inisialisasi variabel pembayaran
    $nama_bank = '';
    $no_rekening = '';
    ...
    // Validasi role (sesuai dengan enum di database)
    if (!in_array($role, ['penjual', 'pembeli'])) {
        $errors[] = "Role tidak valid.";
    }
    
    // Validasi untuk penjual
    if ($role == 'penjual') {
        $nama_bank = trim($_POST['nama_bank'] ?? '');
        $no_rekening = trim($_POST['no_rekening'] ?? '');
        
        if (empty($nama_bank)) {
            $errors[] = "Nama bank wajib diisi untuk penjual.";
        }
        
        if (empty($no_rekening) || !is_numeric($no_rekening)) {
            $errors[] = "Nomor rekening wajib diisi dan harus angka.";
        }
    }
    
    // Cek apakah NIK sudah terdaftar di tabel users
    if (empty($errors)) {
    ...
    if (empty($errors)) {
        ...
        // Default gambar (avatar berdasarkan role)
        $default_gambar = ($role == 'penjual') ? 'default_seller.png' : 'default_buyer.png';
        
        // Query untuk insert data ke tabel USERS
        if ($role == 'penjual') {
            $query = "INSERT INTO users (nik, username, password, email, role, status, gambar, alamat, nama_bank, no_rekening, created_at) 
                    VALUES (?, ?, ?, ?, ?, 'aktif', ?, ?, ?, ?, NOW())";
            
            $stmt = $conn->prepare($query);
            $stmt->bind_param("sssssssss", $nik, $username, $hashed_password, $email, $role, $default_gambar, $alamat, $nama_bank, $no_rekening);
        } else {
        ...
        if ($stmt->execute()) {
            // Set session untuk notifikasi di halaman login
            session_start();
            $_SESSION['register_success'] = "Pendaftaran berhasil! Akun $role telah dibuat untuk $username. Silakan login.";
    ...
    <div class="container">        
        <div class="form-container">
            ...
            <form id="registerForm" method="POST" action="">
                ...
                <!-- Input khusus untuk penjual - HANYA MUNCUL JIKA ROLE PENJUAL DIPILIH -->
                <div id="penjualFields" class="penjual-fields <?php echo (isset($_POST['role']) && $_POST['role'] == 'penjual') ? 'show' : ''; ?>">
                    <h3 style="color: var(--primary-blue); margin-bottom: 15px;">Informasi Pembayaran Penjual</h3>
                    
                    <div class="input-group">
                        <label for="nama_bank">Nama Bank</label>
                        <div class="input-with-icon">
                            <i class="fas fa-university"></i>
                            <input type="text" id="nama_bank" name="nama_bank" placeholder="Contoh: BCA, Mandiri, BRI"
                                   value="<?php echo isset($_POST['nama_bank']) ? htmlspecialchars($_POST['nama_bank']) : ''; ?>">
                        </div>
                    </div>
                    ...
                    <div class="input-group">
                        ...
                        <div class="input-with-icon">
                            ...
                            <input type="text" id="no_rekening" name="no_rekening" placeholder="Masukkan nomor rekening"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('registerForm').addEventListener('submit', (e) => {
                ...
                if (role === 'penjual') {
                    if (!nama_bank) {
                        e.preventDefault();
                        showError('Nama bank wajib diisi untuk penjual.');
                        return;
                    }
                    
                    if (!no_rekening) {
                        e.preventDefault();
                        showError('Nomor rekening wajib diisi untuk penjual.');
                        return;
                    }
                    
                    const rekeningRegex = /^[0-9]+$/;
                    if (no_rekening && !rekeningRegex.test(no_rekening)) {
                        e.preventDefault();
                        showError('Nomor rekening harus berupa angka.');
                        return;
                    }
                }
            });
